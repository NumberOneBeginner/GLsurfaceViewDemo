	...
	public Camera openCamera(boolean isBackCamera, Activity activity,
		...
		try {
			if (isBackCamera)
				...
				cameraId = 1;
			int width = Util.getPhoneWidth();
			int height = Util.getPhoneHeight();
...
//			 mCamera.getParameters(), Screen.mWidth, Screen.mHeight);
			Size bestPreviewSize = calBestPreviewSize(
	...
	public static ArrayList<HashMap<String, Integer>> getCameraPreviewSize(
		...
		try {
			...
			List<Size> allSupportedSize = camera.getParameters()
	 ...
	 */
	private Size calBestPreviewSize(Camera.Parameters camPara,
	...
	public Bitmap getBitMap(byte[] data, Camera camera, boolean mIsFrontalCamera) {
		...
		tmpBitmap.compress(Bitmap.CompressFormat.JPEG, 80, out);
//		float zoom = (float)Math.sqrt(20 * 1024 / (float)out.toByteArray().length);
		matrix.setScale(0.9f, 0.9f);
		Bitmap result = Bitmap.createBitmap(tmpBitmap, 0, 0, tmpBitmap.getWidth(), tmpBitmap.getHeight(), matrix, true);
		out.reset();
		result.compress(Bitmap.CompressFormat.JPEG, 80, out);
		while(out.toByteArray().length > 13 * 1024){
