...
public class CameraMatrix {
	// GLSL 程序: 和普通的 view 利用 canvas 来绘制不一样，OpenGL 需要加载 GLSL 程序，让 GPU
	// 进行绘制。所以我们需要定义 shader 代码

	/**
	 * GLSL 我们的 Java 代码需要获取 shader 代码中定义的变量索引，并在稍后的绘制代码中进行赋值。
	 * 
	 * 创建 GLSL 程序：mProgram = GLES20.glCreateProgram() 加载 shader 代码：vertexShader
	 * = loadShader(GLES20.GL_VERTEX_SHADER, VERTEX_SHADER) attatch shader
	 * 代码：GLES20.glAttachShader(mProgram, vertexShader) 链接 shader
	 * 代码：GLES20.glLinkProgram(mProgram) 获取 shader 代码中的变量索引：mPositionHandle =
	 * GLES20.glGetAttribLocation(mProgram, "vPosition") 需要指出的是，shader代码中的变量索引，在
	 * GLSL 程序的生命周期内（两次链接之间），都是固定的，只需要获取一次。
	 * 
	 */
	// vertex着色器code
